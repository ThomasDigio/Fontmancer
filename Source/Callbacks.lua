...
function Fontmancer:HookSpacingCallback(metaTable)
    hooksecurefunc(metaTable, "SetSpacing", function(fontInstance, spacing, isFontmancerCall)
        if not isFontmancerCall then
            local fontName = fontInstance:GetName()
            if not self.originalFonts[fontName] then
                self.originalFonts[fontName] = {}
...
function Fontmancer:HookShadowOffsetCallback(metaTable)
    hooksecurefunc(metaTable, "SetShadowOffset", function(fontInstance, x, y, isFontmancerCall)
        if not isFontmancerCall then
            local fontName = fontInstance:GetName()
            if not self.originalFonts[fontName] then
                self.originalFonts[fontName] = {}
...
function Fontmancer:HookIndentCallback(metaTable)
    hooksecurefunc(metaTable, "SetIndentedWordWrap", function(fontInstance, indent, isFontmancerCall)
        if not isFontmancerCall then
            local fontName = fontInstance:GetName()
            if not self.originalFonts[fontName] then
                self.originalFonts[fontName] = {}
